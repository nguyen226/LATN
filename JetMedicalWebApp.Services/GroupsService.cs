...
namespace JetMedicalWebApp.Services
{
    public class GroupsService
    {
        ...
        private string maTable = "Groups";
        private string tenTable = "Groups";

        public List<GroupsDto> GetListExposeDto(Dictionary<string, string> filters, Dictionary<string, string> inData,
                                                        out Dictionary<string, string> outData)
        {
            ...
            var query = unitOfWork.GroupsRepository.Get().Select("new (" + inData[CommonConstants.StrSelectedFields] + ")");
        ...
        public IEnumerable<GroupsDto> GetAllExposeDto(string selectedFields, string sortedColumnNames)
        {
        ...
        public GroupsDto GetByIdExposeDto(int id, string selectedFields)
        {
            Dictionary<string, string> filters = new Dictionary<string, string>();
            Dictionary<string, string> inData = new Dictionary<string, string>();
            Dictionary<string, string> outData = null;
            ...
            string stringFilter = "GroupID=" + id.ToString();
        ...
        {
            return unitOfWork.GroupsRepository.Get().Select("new (" + selectedFields + ")")
                                                    .Where(stringFilter)
                                                    .Count();
        }
        ...
        {
            return unitOfWork.GroupsRepository.Get(m => m.GroupName.Trim().ToLower().Equals(name.Trim().ToLower())).FirstOrDefault() != null;
        }
        ...
        public string DeleteByIds(List<int> ids)
        {
            ...
            if (ids.Count() > 0)
            {
                ...
                try
                {
                    var query = unitOfWork.GroupsRepository.Get(m => ids.Contains(m.GroupID)).ToList();
                    if (query != null)
                    {
                        appHistoryServiceService.Add(new AppHistory()
                        {
                            ...
                            GhiChu = "GroupID: " + string.Join(";", ids)
                        });
                      
                        foreach(var item in query)
                        {
                            unitOfWork.GroupsRepository.Delete(item);
                            unitOfWork.Save();
                        }
                    }
                }
        ...
        public string UpdateByIds(Dictionary<int, Dictionary<string, string>> updatedData)
        {
            ...
            if (updatedData != null)
            {
                ...
                foreach (Groups obj in query)
                {
                    foreach (KeyValuePair<string, string> item in updatedData[obj.GroupID])
                    {
                        switch (item.Key)
                        {
                            default:
                        ...
                        appHistoryServiceService.Add(new AppHistory()
                        {
                            ...
                            GhiChu = "GroupID: " + obj.GroupID.ToString() + ", field: " + item.Key
                        });
        ...
        public string AddOrUpdate(string id, Dictionary<string, string> updatedValues)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(id))
                {
                    ...
                    {
                        resultMessage = "Giá trị GroupID=" + id + " không phù hợp.";
                    }
                }
                else
                {
                    nguoiSuDungCapNhat = internalService.GetNguoiSuDungCapNhat(unitOfWork);
                    if (IsExitting(updatedValues["GroupName"]))
                    {
                        resultMessage = "Đã tồn tại nhóm người dùng này.";
                    }
                    else
                    {
                        Groups = new Groups()
                        {
                            GroupName = updatedValues["GroupName"],
                            Notes = updatedValues.ContainsKey("Notes") ? updatedValues["Notes"] : string.Empty,
                            Active = updatedValues.ContainsKey("Active") ? (bool.TryParse(updatedValues["Active"], out boolValue) ? boolValue : false) : false,
                            ModifiedUsers = nguoiSuDungCapNhat,
                            CreatedUsers = nguoiSuDungCapNhat
                        };
                        Groups = unitOfWork.GroupsRepository.Insert(Groups);

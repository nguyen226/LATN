...
namespace JetMedicalWebApp.Services
{
    public class KhoaService
    {
        ...
        private string maTable = "Khoa";
        private string tenTable = "Khoa";

        public List<KhoaDto> GetListExposeDto(Dictionary<string, string> filters, Dictionary<string, string> inData,
                                                        out Dictionary<string, string> outData)
        {
            List<KhoaDto> result = new List<KhoaDto>();
            string stringFilter = string.Empty;
            outData = new Dictionary<string, string>();

            var query = unitOfWork.KhoaRepository.Get().Select("new (" + inData[CommonConstants.StrSelectedFields] + ")");
        ...
        public IEnumerable<KhoaDto> GetAllExposeDto(string selectedFields, string sortedColumnNames)
        {
        ...
        public KhoaDto GetByIdExposeDto(int id, string selectedFields)
        {
            Dictionary<string, string> filters = new Dictionary<string, string>();
            Dictionary<string, string> inData = new Dictionary<string, string>();
            Dictionary<string, string> outData = null;
        ...
        {
            return unitOfWork.KhoaRepository.Get().Select("new (" + selectedFields + ")")
                                                    .Where(stringFilter)
                                                    .Count();
        }
        ...
        {
            return unitOfWork.KhoaRepository.Get(m => m.Name.Trim().ToLower().Equals(name.Trim().ToLower())).FirstOrDefault() != null;
        }
        ...
        public string DeleteByIds(List<int> ids)
        {
            ...
            if (ids.Count() > 0)
            {
                ...
                try
                {
                    var query = unitOfWork.KhoaRepository.Get(m => ids.Contains(m.Id)).ToList();
                    if (query != null)
                    {
                        ...
                        {
                            unitOfWork.KhoaRepository.Delete(item);
                            unitOfWork.Save();
                        }
                    }
                }
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        khoa = unitOfWork.KhoaRepository.Insert(khoa);

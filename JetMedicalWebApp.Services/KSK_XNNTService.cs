...
{
    public class KSK_XNNTService
    {
        private UnitOfWork unitOfWork = new UnitOfWork();

        private string maTable = "KSK_XNNT";
        private string tenTable = "KSK_XNNT";

        public List<KSK_XNNTDto> GetListExposeDto(Dictionary<string, string> filters, Dictionary<string, string> inData,
                                                        out Dictionary<string, string> outData)
        {
            List<KSK_XNNTDto> result = new List<KSK_XNNTDto>();
            string stringFilter = string.Empty, usernameNguoiCapNhat = string.Empty;
            outData = new Dictionary<string, string>();

            var query = unitOfWork.KSK_XNNTRepository.Get().Select("new (" + inData[CommonConstants.StrSelectedFields] + ")");
            ...
            if (filters != null)
            {
                foreach (KeyValuePair<string, string> item in filters)
                {
                    switch (item.Key)
                    {
                        ...
                        default:
                            KSK_XNNTDto obj = new KSK_XNNTDto();
        ...
        public IEnumerable<KSK_XNNTDto> GetAllExposeDto(string selectedFields, string sortedColumnNames)
        {
        ...
        public KSK_XNNTDto GetByIdExposeDto(int id, string selectedFields)
        {
            Dictionary<string, string> filters = new Dictionary<string, string>();
            Dictionary<string, string> inData = new Dictionary<string, string>();
            Dictionary<string, string> outData = null;
        ...
        public KSK_XNNTDto GetByUsernameExposeDto(string username, string selectedFields)
        {
            Dictionary<string, string> filters = new Dictionary<string, string>();
            Dictionary<string, string> inData = new Dictionary<string, string>();
            Dictionary<string, string> outData = null;
        ...
        public KSK_XNNTDto GetByEmailOrPhoneExposeDto(string username, string selectedFields)
        {
            Dictionary<string, string> filters = new Dictionary<string, string>();
            Dictionary<string, string> inData = new Dictionary<string, string>();
            Dictionary<string, string> outData = null;
        ...
        public KSK_XNNTDto GetByUserIdExposeDto(string userId, string selectedFields)
        {
            Dictionary<string, string> filters = new Dictionary<string, string>();
            Dictionary<string, string> inData = new Dictionary<string, string>();
            Dictionary<string, string> outData = null;
        ...
        public int CountDataRow(string selectedFields, string stringFilter)
        {
            ...
            {
                return unitOfWork.KSK_XNNTRepository.Get().Select("new (" + selectedFields + ")")
                                                    .Where(stringFilter)
                                                    .Count();
            }
            return unitOfWork.KSK_XNNTRepository.Get().Select("new (" + selectedFields + ")").Count();
        }
        public string DeleteByIds(List<int> ids)
        {
            ...
            if (ids.Count() > 0)
            {
                ...
                try
                {
                    ...
                    var query = unitOfWork.KSK_XNNTRepository.Get(m => ids.Contains(m.KSK_XNNT_ID)).ToList();
                    if (query != null)
                    {
                        ...
                        unitOfWork.KSK_XNNTRepository.DeleteRange(query);
                        unitOfWork.Save();
                    }
                }
                catch (Exception ex)
        ...
        public string UpdateByIds(Dictionary<int, Dictionary<string, string>> updatedData)
        {
            ...
            if (updatedData != null)
            {
                ...
                AppHistoryService appHistoryService = new AppHistoryService();
                var query = unitOfWork.KSK_XNNTRepository.Get(m => ids.Contains(m.KSK_XNNT_ID)).ToList();
                foreach (KSK_XNNT obj in query)
                {
                    foreach (KeyValuePair<string, string> item in updatedData[obj.KSK_XNNT_ID])
                    {
                        switch (item.Key)
                        {
                            default:
                        ...
                        appHistoryService.Add(new AppHistory()
                        {
                            ...
                            GhiChu = "UserID: " + obj.KSK_XNNT_ID.ToString() + ", field: " + item.Key
                        });
                    }
                }
        ...
        public string AddOrUpdate(string id, Dictionary<string, string> updatedValues)
        {
            ...
            try
            {
                ...
                {
                    KSK_XNNT = new KSK_XNNT()
                    {
                        MA_LK = updatedValues.ContainsKey("MA_LK") ? updatedValues["MA_LK"] : string.Empty,
                        TEN_XNNT = updatedValues.ContainsKey("TEN_XNNT") ? updatedValues["TEN_XNNT"] : string.Empty,
                        KQ_XNTT = updatedValues.ContainsKey("KQ_XNTT") ? updatedValues["KQ_XNTT"] : string.Empty,
                    };

                    KSK_XNNT = unitOfWork.KSK_XNNTRepository.Insert(KSK_XNNT);
        ...
        public void ThucHienImport(int tuDong, int toiDong, DataTable dt, int totalColumn)
        {
            ...
            for (int n = tuDong; n < toiDong; n++)
            {
                for (int c = 1; c < totalColumn; c++)
                {
                    ...
                    string TEN_XNNT = dt.Columns[c].ToString();
                    string KQ_XNTT = dt.Rows[n][c].ToString();

                    unit3.KSK_XNNTRepository.Insert(new KSK_XNNT()
                    {
                        MA_LK = MA_LK,
                        TEN_XNNT = TEN_XNNT,
                        KQ_XNTT = KQ_XNTT
        ...
        public List<KSK_XNNTDto> GetListTTBNDataFromViewExposeDto(Dictionary<string, string> filters, Dictionary<string, string> inData,
                                                                       out Dictionary<string, string> outData)
        {
            List<KSK_XNNTDto> result = new List<KSK_XNNTDto>();
            string tableName = "uv_KhamSucKhoeXNNT";
            ...
            var displayedData = unitOfWork.DataContext.Database.SqlQuery<KSK_XNNTDto>(sqlCommand + (!string.IsNullOrEmpty(sortedColumnNames) ? " ORDER BY " + sortedColumnNames : "") + " " + paging).AsEnumerable();

...
namespace JetMedicalWebApp.Services
{
    public class ServiceService
    {
        ...
        private string maTable = "Service";
        private string tenTable = "Service";

        public List<ServiceDto> GetListExposeDto(Dictionary<string, string> filters, Dictionary<string, string> inData,
                                                        out Dictionary<string, string> outData)
        {
            ...
            var query = unitOfWork.ServiceRepository.Get().Select("new (" + inData[CommonConstants.StrSelectedFields] + ")");
        ...
        public IEnumerable<ServiceDto> GetAllExposeDto(string selectedFields, string sortedColumnNames)
        {
        ...
        public ServiceDto GetByIdExposeDto(int id, string selectedFields)
        {
            Dictionary<string, string> filters = new Dictionary<string, string>();
            Dictionary<string, string> inData = new Dictionary<string, string>();
            Dictionary<string, string> outData = null;
        ...
        {
            return unitOfWork.ServiceRepository.Get().Select("new (" + selectedFields + ")")
                                                    .Where(stringFilter)
                                                    .Count();
        }
        ...
        public string DeleteByIds(List<int> ids)
        {
            ...
            if (ids.Count() > 0)
            {
                ...
                try
                {
                    ...
                    var query = unitOfWork.ServiceRepository.Get(m => ids.Contains(m.id)).ToList();
                    if (query != null)
                    {
                        ...
                        {
                            if(item.Service_Tags != null)
                            {
                                if(item.Service_Tags.Count() > 0)
                                {
                                    unitOfWork.Service_TagRepository.DeleteRange(item.Service_Tags);
                                }
                            }

                            unitOfWork.ServiceRepository.Delete(item);
                        }
        ...
        {
            return unitOfWork.ServiceRepository.Get(m => m.name.Trim().ToLower().Equals(name.Trim().ToLower())).FirstOrDefault() != null;
        }
                    ...
                    foreach (KeyValuePair<string, string> item in updatedData[obj.id])
                    {
                        switch (item.Key)
                        {
                            ...
                            case "Tags":
                                ...
                                if (tags.Length > 0)
                                {
                                    unitOfWork.Service_TagRepository.DeleteRange(obj.Service_Tags);
                                    unitOfWork.Save();
                                    foreach (var tag in tags)
                                    {
                                        ...
                                        Service_Tag tinTuc_TagInsert = new Service_Tag();
                                        tinTuc_TagInsert.Service = obj;
                                        tinTuc_TagInsert.Tag = insertTag;
                                        obj.Tags = item.Value;
                                        tinTuc_TagInsert = unitOfWork.Service_TagRepository.Insert(tinTuc_TagInsert);
        ...
        public string AddOrUpdate(ServiceDto item)
        {
            Service service = null;
            string resultMessage = string.Empty;
            int categoriIdValue, nViewValue, positionValue, priceSaleValue, priceOldValue, isGroupValue, 
                ...
                if (!string.IsNullOrEmpty(item.name))
                {
                    ...
                    foreach (System.Reflection.PropertyInfo itemPropertyInfo in item.GetType().GetProperties())
                    {
                        ...
                        switch (itemPropertyInfo.Name)
                        {
                            ...
                            case "code":
                                if (itemPropertyInfo.GetValue(item, null) != null && !string.IsNullOrEmpty(itemPropertyInfo.GetValue(item, null).ToString()))
                                {
                                    code = itemPropertyInfo.GetValue(item, null).ToString();
                                }
                    ...
                    else
                    {
                        ...
                        service = unitOfWork.ServiceRepository.Insert(service);
                        ...
                        resultMessage = service.id.ToString();
                        ...
                        if (updatedValues.ContainsKey("Tags"))
                        {
                            if (!string.IsNullOrEmpty(updatedValues["Tags"]))
                            {
                                ...
                                foreach (var tag in tags)
                                {
                                    ...
                                    Service_Tag tinTuc_TagInsert = new Service_Tag();
                                    tinTuc_TagInsert.Service = service;
                                    tinTuc_TagInsert.Tag = insertTag;
                                    tinTuc_TagInsert = unitOfWork.Service_TagRepository.Insert(tinTuc_TagInsert);
                                    unitOfWork.Save();
                                }
        ...
        public List<ServiceDto> GetListDataFromViewExposeDto(Dictionary<string, string> filters, Dictionary<string, string> inData,
                                                                   out Dictionary<string, string> outData)
        {
            List<ServiceDto> result = new List<ServiceDto>();
            string tableName = "uv_Service";
            string stringFilter = string.Empty;
            outData = new Dictionary<string, string>();
            ...
            var displayedData = unitOfWork.DataContext.Database.SqlQuery<ServiceDto>(sqlCommand + (!string.IsNullOrEmpty(sortedColumnNames) ? " ORDER BY " + sortedColumnNames : "") + " " + paging).AsEnumerable();
        ...
        public List<Tag> GetListTagByTinTucId(int id)
        {
            ...
            var TinTuc_Tag = unitOfWork.Service_TagRepository.Get(m => m.Service.id == id);
            if (TinTuc_Tag != null)
            {
        ...
        {
            var news = unitOfWork.ServiceRepository.GetByID(id);
            news.nView += 1;
            unitOfWork.Save();
        }
    }
